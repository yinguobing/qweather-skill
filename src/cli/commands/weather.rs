...
pub async fn execute(client: &QWeatherClient, cmd: &WeatherSubcommand, lang: &str) -> Result<()> {
    ...
    match cmd {
        ...
        WeatherSubcommand::Daily { days, location } => {
            let resolved = resolve_location(location, client).await?;
            let resp = weather
                .daily_forecast(&resolved.id, *days, lang, None)
                .await?;
        ...
        WeatherSubcommand::Hourly { hours, location } => {
            let resolved = resolve_location(location, client).await?;
            let resp = weather
                .hourly_forecast(&resolved.id, *hours, lang, None)
                .await?;
